...
public class GsmCellBroadcastHandler extends CellBroadcastHandler {
    ...
    public final SparseArray<String> mAreaInfos = new SparseArray<>();
    private BroadcastReceiver mReceiver;
    private final HashMap<SmsCbConcatInfo, byte[][]> mSmsCbPageMap = new HashMap<>(4);

    public GsmCellBroadcastHandler(Context context, Looper looper, CellBroadcastHandler.CbSendMessageCalculatorFactory cbSendMessageCalculatorFactory) {
        super("GsmCellBroadcastHandler", context, looper, cbSendMessageCalculatorFactory);
        C00061 r2 = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                String action = intent.getAction();
                if (((action.hashCode() == 607720192 && action.equals("com.android.cellbroadcastreceiver.action.AREA_UPDATE_INFO_ENABLED")) ? (char) 0 : 65535) != 0) {
                    GsmCellBroadcastHandler.this.log("Unhandled broadcast " + intent.getAction());
                    return;
                }
                boolean booleanExtra = intent.getBooleanExtra("enable", false);
                GsmCellBroadcastHandler.this.log("Area update info enabled: " + booleanExtra);
                String[] stringArray = GsmCellBroadcastHandler.this.mContext.getResources().getStringArray(R.array.config_area_info_receiver_packages);
                if (!booleanExtra) {
                    for (int i = 0; i < GsmCellBroadcastHandler.this.mAreaInfos.size(); i++) {
                        int keyAt = GsmCellBroadcastHandler.this.mAreaInfos.keyAt(i);
                        GsmCellBroadcastHandler.this.log("Area update info disabled, clear areaInfo from: " + ((String) GsmCellBroadcastHandler.this.mAreaInfos.get(keyAt)));
                        GsmCellBroadcastHandler.this.mAreaInfos.put(keyAt, (Object) null);
                    }
                }
                for (int i2 = 0; i2 < GsmCellBroadcastHandler.this.mAreaInfos.size(); i2++) {
                    int keyAt2 = GsmCellBroadcastHandler.this.mAreaInfos.keyAt(i2);
                    for (String str : stringArray) {
                        Intent intent2 = new Intent("android.telephony.action.AREA_INFO_UPDATED");
                        intent.putExtra("android.telephony.extra.SLOT_INDEX", keyAt2);
                        intent.putExtra("enable", booleanExtra);
                        intent.setPackage(str);
                        GsmCellBroadcastHandler.this.mContext.sendBroadcastAsUser(intent2, UserHandle.ALL, "android.permission.READ_PRIVILEGED_PHONE_STATE");
                    }
                }
            }
        };
        this.mReceiver = r2;
        this.mContext.registerReceiver(r2, new IntentFilter("com.android.cellbroadcastreceiver.action.AREA_UPDATE_INFO_ENABLED"), "com.android.cellbroadcastservice.FULL_ACCESS_CELL_BROADCAST_HISTORY", (Handler) null);
    }
    ...
    public static GsmCellBroadcastHandler makeGsmCellBroadcastHandler(Context context) {
        GsmCellBroadcastHandler gsmCellBroadcastHandler = new GsmCellBroadcastHandler(context, Looper.myLooper(), new CellBroadcastHandler.CbSendMessageCalculatorFactory());
    ...
    private boolean handleGeoFencingTriggerMessage(GsmSmsCbMessage.GeoFencingTriggerMessage geoFencingTriggerMessage, int i) {
        ...
        requestLocationUpdate(new CellBroadcastHandler.LocationUpdateCallback(arrayList, arrayList2, i, arrayList3) {
            ...
            public final void onLocationUpdate(CbGeoUtils.LatLng latLng, double d) {
                GsmCellBroadcastHandler.this.lambda$handleGeoFencingTriggerMessage$0$GsmCellBroadcastHandler(this.f$1, this.f$2, this.f$3, this.f$4, latLng, d);
            }
        }, i2);
        return true;
        throw th;
    ...
    public /* synthetic */ void lambda$handleGeoFencingTriggerMessage$0$GsmCellBroadcastHandler(List list, List list2, int i, List list3, CbGeoUtils.LatLng latLng, double d) {
        List list4;
        List list5 = list;
        List list6 = list2;
        int i2 = i;
        if (latLng == null) {
            for (int i3 = 0; i3 < list.size(); i3++) {
                broadcastMessage((SmsCbMessage) list.get(i3), (Uri) list2.get(i3), i2);
            }
            return;
        }
        for (int i4 = 0; i4 < list.size(); i4++) {
            if (!list3.isEmpty()) {
                ...
                list4 = ((SmsCbMessage) list.get(i4)).getGeometries();
            }
            if (list4 == null || list4.isEmpty()) {
                broadcastMessage((SmsCbMessage) list.get(i4), (Uri) list2.get(i4), i2);
            } else {
                performGeoFencing((SmsCbMessage) list.get(i4), (Uri) list2.get(i4), list4, latLng, i, d);
            }
        }
    }
